...
def fastrcnn_loss(class_logits, box_regression, attr_logits, labels, regression_targets, matched_gt_attr):
    ...
    #print([i for i in labels if i > 0])
    classification_loss = F.cross_entropy(class_logits, labels)
    #attr_classification_loss = F.cross_entropy(attr_logits, attr_labels)
    ...
    # This Label Pos has the class values of gt label
    # For Attribute classification: Use sampled pos inds to index into
    # attr logits,
    #print("Sampled psd ind: ", sampled_pos_inds_subset.shape, sampled_pos_inds_subset)
    #print("label pos", labels_pos)
    attr_labels = torch.cat(matched_gt_attr, dim=0)
    compare_attr_logits = attr_logits[sampled_pos_inds_subset]
    compare_attr_labels = attr_labels[sampled_pos_inds_subset]
    compare_attr_labels = compare_attr_labels.to(dtype=torch.float32)
    attr_classification_loss = F.binary_cross_entropy_with_logits(compare_attr_logits, compare_attr_labels)
    #attr_classification_loss = 0
    #attr_loss =
    #print(compare_attr_logits.shape)
    #print(compare_attr_labels.shape)
    N, num_classes = class_logits.shape
    box_regression = box_regression.reshape(N, -1, 4)
    #print("box Reg: ", box_regression.shape)
    ...
    return classification_loss, box_loss, attr_classification_loss
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            #print(gt_attr_in_image.shape)
            #print(gt_attr_in_image)
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                #print(matched_idxs_in_image)
                #print(matched_idxs_in_image.shape)
                #This has the information of which idx matches with which gt labels
                clamped_matched_idxs_in_image = matched_idxs_in_image.clamp(min=0)
                #print(clamped_matched_idxs_in_image)
                #print("Clamped Idx:", [i for i in clamped_matched_idxs_in_image if i > 0])
                #print(clamped_matched_idxs_in_image.shape)
                # For All proposal assign a label to them based on index
                # labels in image = number of proposals..
                labels_in_image = gt_labels_in_image[clamped_matched_idxs_in_image]
                #attr_in_image = gt_attr_in_image[clamped_matched_idxs_in_image]
                #print(attr_in_image.shape)
                labels_in_image = labels_in_image.to(dtype=torch.int64)


                # Label background (below the low threshold)
                bg_inds = matched_idxs_in_image == self.proposal_matcher.BELOW_LOW_THRESHOLD
                #print("Low Threshold: ",self.proposal_matcher.BELOW_LOW_THRESHOLD)
                # This will make a lot of them 0 for background (Below Low T = -1)
                labels_in_image[bg_inds] = torch.tensor(0)

                # Label ignore proposals (between low and high thresholds)
                ignore_inds = matched_idxs_in_image == self.proposal_matcher.BETWEEN_THRESHOLDS
                #print("Between Threshold: ", self.proposal_matcher.BETWEEN_THRESHOLDS)
                # Betwenn T = -2, all of them are set to -1 and ignored by sampler
                labels_in_image[ignore_inds] = torch.tensor(-1)  # -1 is ignored by sampler
                #print("Ignore Index", len([i for i in labels_in_image if i == -1]))
    ...
    def subsample(self, labels):
        ...
        #print("Positive Sample: ", [i for i in sampled_pos_inds[0] if i > 0])
        #print("Negative Sample: ", len([i for i in sampled_neg_inds[0] if i > 0]))
        #print("Positive Sample1: ", [i for i in sampled_pos_inds[1] if i > 1])
        #print("Negative Sample1: ", len([i for i in sampled_neg_inds[1] if i > 1]))
        sampled_inds = []
    ...
    def select_training_samples(self,
        ...
        gt_attr_labels = [t["attributes"] for t in targets]
        ...
        for img_id in range(num_images):
            ...
            #print("Img: ", img_id, "  Labels: ", labels[img_id])
            #attr_labels[img_id] = attr_labels[img_id][img_sampled_inds]
            matched_idxs[img_id] = matched_idxs[img_id][img_sampled_inds]
            gt_attr_in_image = gt_attr_labels[img_id]
            if gt_attr_in_image.numel() == 0:
              gt_attr_in_image = torch.zeros((1, 341), dtype=dtype, device=device)
            matched_gt_attr.append(gt_attr_in_image[matched_idxs[img_id]])
            #print(len(matched_gt_attr))
            #print(matched_gt_attr[img_id].shape)
            #print("Matched Attr: ", matched_gt_attr[img_id])
        ...
        return proposals, matched_idxs, labels, regression_targets, matched_gt_attr
    ...
    def postprocess_detections(self,
        ...
        all_attr_scores = []
        all_attr_labels = []
        for boxes, scores, attr_scores, image_shape in zip(pred_boxes_list, pred_scores_list, pred_attr_scores_list, image_shapes):
            ...
            new_attr_shape = (detection_per_image, num_classes-1, attr_classes)
            #print(attr_scores)
            ...
            #print(inds)
            boxes, scores, labels, attr_scores, attr_labels = boxes[inds], scores[inds], labels[inds], attr_scores[inds], attr_labels[inds]

            # remove empty boxes
            keep = box_ops.remove_small_boxes(boxes, min_size=1e-2)
            boxes, scores, labels, attr_scores, attr_labels = boxes[keep], scores[keep], labels[keep], attr_scores[keep], attr_labels[keep]
    ...
    def forward(self,
        ...
        if self.training:
            proposals, matched_idxs, labels, regression_targets, matched_gt_attr = self.select_training_samples(proposals, targets)
        else:
            labels = None
            regression_targets = None
            matched_idxs = None
            matched_gt_attr = None

        box_features = self.box_roi_pool(features, proposals, image_shapes)
        box_features = self.box_head(box_features)
        class_logits, box_regression, attr_logits = self.box_predictor(box_features)
        #print(class_logits.shape)
        #print(attr_logits)
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg, loss_attr_classifier = fastrcnn_loss(
                class_logits, box_regression, attr_logits, labels, regression_targets, matched_gt_attr)
            losses = {
            ...
            boxes, scores, labels, attr_scores, attr_labels = self.postprocess_detections(class_logits, box_regression, proposals, attr_logits, image_shapes)
                ...
                result.append(
                    {
                        ...
                        "labels": labels[i],
                        "scores": scores[i],
                        "attr_labels": attr_labels[i],
                        "attr_scores": attr_scores[i]
                    }
                )
