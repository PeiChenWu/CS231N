...
from .trend_rcnn import TrendRCNNOutputLayers
...
@ROI_HEADS_REGISTRY.register()
class TrendRCNNROIHeads(StandardROIHeads):
    """
    The ROI specific heads for Trend R-CNN
    """

    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        super().__init__(cfg, input_shape)
        self._init_box_head_2(cfg, input_shape)
    ...
    def _init_box_head_ignore(self, cfg, input_shape):
        # fmt: off
        in_features       = cfg.MODEL.ROI_HEADS.IN_FEATURES
        pooler_resolution = cfg.MODEL.ROI_BOX_HEAD.POOLER_RESOLUTION
        pooler_scales     = tuple(1.0 / input_shape[k].stride for k in in_features)
        ...
        box_predictor = TrendRCNNOutputLayers(cfg, box_head.output_shape)
        return {
            "box_in_features": in_features,
            "box_pooler": box_pooler,
            "box_head": box_head,
    ...
    def _init_box_head_2(self, cfg, input_shape):
        self.box_predictor = TrendRCNNOutputLayers(cfg, self.box_head.output_shape)


@ROI_HEADS_REGISTRY.register()
class TrendRes5ROIHeads(Res5ROIHeads):
    """
    The ROIHeads in a typical "C4" R-CNN model, where
    the box and mask head share the cropping and
    the per-region feature computation by a Res5 block.
    ...
    def _init_box_head_2(self, cfg, input_shape):
        self.res5, out_channels = self._build_res5_block(cfg)
        self.box_predictor = TrendRCNNOutputLayers(
            cfg, ShapeSpec(channels=out_channels, height=1, width=1)
        )
