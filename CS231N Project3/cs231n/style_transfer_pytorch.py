...
def gram_matrix(features, normalize=True):
    ...
    reshpae_features = features.reshape(1, C, H*W)

    gram_m = torch.mm(reshpae_features[0,:,:],reshpae_features[0,:,:].transpose(1,0))
    
    if normalize:
      gram_m /= (H*W*C)

    gram_m = gram_m.unsqueeze(0)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    style_loss = torch.FloatTensor([0])
    
    for index,layer in enumerate(style_layers):
      image_gram_cur = gram_matrix(feats[layer])
      style_loss += style_weights[index] * torch.sum((image_gram_cur - style_targets[index])**2)
